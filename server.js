...
app.get('/api/ml/ads-custo-lucro', async (req, res) => {
  ...
  try {
    // 1. SKU e preço atual de cada produto (multiget em lotes de 20)
    const skuPorMlb   = {};
    const precoPorMlb = {};
    for (let i = 0; i < mlbs.length; i += 20) {
      const chunk = mlbs.slice(i, i + 20);
      const r = await axios.get('https://api.mercadolibre.com/items', {
        params: { ids: chunk.join(','), attributes: 'id,price,seller_custom_field,attributes,variations' },
        headers, timeout: 15000,
      });
      r.data.forEach(item => {
        if (item.code === 200) {
          skuPorMlb[item.body.id]   = extrairSkuDoItem(item.body);
          precoPorMlb[item.body.id] = item.body.price ?? item.body.variations?.[0]?.price ?? null;
        }
      });
    }
...
app.get('/api/ml/debug-ads', async (req, res) => {
  ...
  const tryGet = async (label, url, params = {}, extraHeaders = {}) => {
    try {
      const r = await axios.get(url, { headers: { ...headers, ...extraHeaders }, params, timeout: 10000 });
      result[label] = { status: r.status, data: r.data };
    } catch (e) {
      result[label] = { status: e.response?.status, error: e.response?.data || e.message };
    }
  ...
  try {
    ...
    result['_advertisers'] = advResp.data;
    advertiserId = advResp.data?.advertisers?.[0]?.advertiser_id || advResp.data?.[0]?.advertiser_id || null;
    result['_advertiser_id_usado'] = advertiserId || 'nenhum encontrado';
  } catch (e) {
...
app.get('/api/debug/shopee-invoice-pendentes', async (req, res) => {
  ...
  try {
    const accessToken = await getShopeeToken(data);
    const path   = '/api/v2/order/get_pending_buyer_invoice_order_list';
    const params = shopeeParams(path, sp.partner_key, sp.partner_id, accessToken, sp.shop_id);
    params.page_size = 50;
    const r = await axios.get(`${SHOPEE_BASE}/order/get_pending_buyer_invoice_order_list`, { params, timeout: 10000 });
    res.json(r.data);
  } catch (err) {
...
app.get('/api/debug/shopee-probe', async (req, res) => {
  const data = loadData();
  const sp   = data.shopee || {};
  const orderSn = req.query.order_sn;
  if (!orderSn) return res.json({ error: 'order_sn obrigatório' });
  const candidatos = [
    { path: '/api/v2/logistics/get_shipping_parameter', metodo: 'get', params: { order_sn: orderSn } },
    { path: '/api/v2/logistics/create_shipping_document', metodo: 'post', body: { order_list: [{ order_sn: orderSn }] } },
    { path: '/api/v2/logistics/get_shipping_document_result', metodo: 'get', params: { order_sn_list: orderSn } },
    { path: '/api/v2/logistics/download_shipping_document', metodo: 'post', body: { order_list: [{ order_sn: orderSn }] } },
    { path: '/api/v2/order/get_buyer_invoice_info', metodo: 'get', params: { order_sn_list: orderSn } },
  ];
  const resultados = [];
  for (const c of candidatos) {
    ...
    try {
      ...
      if (c.metodo === 'get') {
        ...
        r = await axios.get(`${SHOPEE_BASE}${c.path.replace('/api/v2', '')}`, { params, timeout: 10000 });
      } else {
        r = await axios.post(`${SHOPEE_BASE}${c.path.replace('/api/v2', '')}`, c.body, { params, timeout: 10000 });
      }
      resultados.push({ path: c.path, status: 200, data: r.data });
    } catch (err) {
      resultados.push({ path: c.path, status: err.response?.status, data: err.response?.data || err.message });
    }
  }
  res.json(resultados);
});
...
app.post('/api/contas-receber/sync', async (req, res) => {
  ...
  try {
    ...
    const dateFrom = new Date(Date.now() - dias * 86400000).toISOString().slice(0, 10);
    let todasOrdens = [];
    // Sem filtro de order.status: um pedido pago pode acabar em vários status diferentes
    // (paid, cancelled, partially_refunded, e possivelmente outros que ainda não vimos) —
    // em vez de ficar catando status um por um, busca tudo e deixa o filtro "tem payment"
    // abaixo decidir o que interessa rastrear.
    let offset = 0;
    while (offset < 5000) {
      const params = {
        seller: c.user_id, sort: 'date_desc', limit: 50, offset,
        'order.date_created.from': dateFrom + 'T00:00:00.000-03:00',
      };
      const resp = await axios.get('https://api.mercadolibre.com/orders/search', { params, headers, timeout: 15000 });
      const results = resp.data.results || [];
