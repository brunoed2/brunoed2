...
async function carregarVendas() {
  ...
  try {
    ...
    console.log('[debug vendas] Shopee C1:', shopee1, 'Shopee C2:', shopee2);
    const debugEl = document.getElementById('vendas-debug-canal');
    if (debugEl) {
      debugEl.textContent = `[debug] ML C1: ${(ml1.vendas || []).length} | ML C2: ${(ml2.vendas || []).length}`
        + ` | Shopee C1: ${(shopee1.vendas || []).length} | Shopee C2: ${(shopee2.vendas || []).length}`
